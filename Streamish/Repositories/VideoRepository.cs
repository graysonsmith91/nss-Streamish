    ...
    public class VideoRepository : BaseRepository, IVideoRepository
    {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @"
                          SELECT v.Title, v.Description, v.Url, v.DateCreated, v.UserProfileId, 
                                 u.Name, u.Email, u.ImageUrl AS UserProfileImageUrl, u.DateCreated AS UserProfileDateCreated
                          FROM Video v
        ...
        public Video GetVideoByIdWithComments(int id)
        {
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
                          SELECT v.Id AS VideoId, v.Title, v.Description, v.Url, v.DateCreated, v.UserProfileId AS VideoUserProfileId, 
                                 u.Name, u.Email, u.ImageUrl AS UserProfileImageUrl, u.DateCreated AS UserProfileDateCreated,
                                 c.Id AS CommentId, c.Message, c.UserProfileId AS CommentUserProfileId
                          FROM Video v
                          LEFT JOIN UserProfile u ON u.Id = v.UserProfileId
        ...
        public List<Video> SearchHottest(DateTime dateTime)
        {
            using (var conn = Connection)
            {
                conn.Open();
